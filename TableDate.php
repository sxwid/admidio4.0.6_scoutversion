...
class TableDate extends TableAccess
{
    ...
    {
	// PTABADEN CHANGE 
	// Returns true if Activity or Deadlineend is further away than 2 months
	...
	else
	{
		...
		//$calc_date =  \DateTime::createFromFormat('Y-m-d', $this->getValue('dat_begin'));
	}
	$today = (new DateTime(DATE_NOW));
	$diff = date_diff($today,$calc_date);
	
	//var_dump($diff->days);
	if($diff->days >= 60) //More than 2 Months away
	{
		//echo ' nonig ';
		return true;
	}
