...
else
{
    ...
    foreach($datesResult['recordset'] as $row)
    {
        ...
        $participationPossible = true; // PTABADEN CHANGE
        ...
        //PTABDEN CHANGE
        if($date->allowedToParticipate() || isLeiter())
        {
            $buttonClass = '';
            ...
            if($getViewMode === 'html')
            {
                ...
                // PTABADEN CHANGE
                if (!$date->deadlineExceeded() || isLeiter())
                {
						...
						if ($getView === 'detail')
                        {
                            $outputButtonParticipation = '
                                <button class="btn btn-secondary">
                                    <i class="fas fa-list"></i>' . $gL10n->get('DAT_REGISTRATION_NOT_POSSIBLE') . '</button>';
                        }
                        else
                        {
                            $outputButtonParticipation = '
                                <a class="admidio-icon-link">
                                    <i class="fas fa-list" data-toggle="tooltip" title="'.$gL10n->get('DAT_REGISTRATION_NOT_POSSIBLE').'"></i></a>';
                        }
        ...
        if($getView === 'detail')
        {
            ...
            $page->addHtml('
                ...
                <div class="card-footer">'.
                    ...
                    */
                    admFuncShowCreateChangeInfoById((int) $date->getValue('dat_usr_id_create'), $date->getValue('dat_timestamp_create'),
