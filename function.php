 ...
 *  @return bool Returns **true** if the user is a "Leiter"
 */
function isLeiter($userId = 0)
{
    global $gDb, $gCurrentUser, $gCurrentOrganization, $gProfileFields;
    ...
    $field = (int) $gProfileFields->getProperty('LEITER', 'usf_id');
	$sql = 'SELECT usd_value 
				FROM '. TBL_USER_DATA. ' 
				WHERE usd_usr_id = ? -- $userId
				AND usd_usf_id = ? -- $field';
				
	$statement = $gDb->queryPrepared($sql, array($userId, $field));
 ...
 *  @return Returns either Scoutname or null if non existant
 */
function getScoutName($userId = 0)
{
    global $gCurrentOrganization,  $gCurrentUser, $gProfileFields, $gDb, $gL10n, $gSettingsManager;
    ...
    $field = $gProfileFields->getProperty('PFADINAME', 'usf_id');
	$sql = 'SELECT usd_value 
				FROM '. TBL_USER_DATA. ' 
				WHERE usd_usr_id = ? -- $userId
				AND usd_usf_id = ? -- $field';
	...
	$pfadiname = $statement->fetchColumn();
...
function admFuncShowCreateChangeInfoById($userIdCreated, $timestampCreate, $userIdEdited = 0, $timestampEdited = null)
{
    ...
    if ($timestampCreate)
    {
        if ($userIdCreated > 0)
        {
            ...
            {
				//PTABADEN CHANGE
                if($userCreate->getValue('PFADINAME'))
                {
                  $htmlCreateName = $userCreate->getValue('PFADINAME');
                }
                else
                {
    ...
    if ($timestampEdited)
    {
        if ($userIdEdited > 0)
        {
            ...
            {
				//PTABADEN CHANGE
				if($userEdit->getValue('PFADINAME'))
				{
					$htmlEditName = $userEdit->getValue('PFADINAME');
				}
				else
				{
