        ...
        if($getRoleId === 0 && count($listVisibleRoleArray) > 0)
        {
            ...
            $queryParams = array_merge(
                array(
                    ...
                    (int) $gProfileFields->getProperty('PFADINAME', 'usf_id'),
                    $currUsrId,
                    DATE_NOW
                ),
            ...
            while ($row = $statement->fetch())
            {
                ...
                if (!isset($currentUserId) || $currentUserId !== $usrId)
                {
                    ...
                    if($row['mem_begin'] <= DATE_NOW && $row['mem_end'] >= DATE_NOW)
                    {
                        ...
                        {
                           $activeList[]  = array($usrId, $row['last_name'].' '.$row['first_name']. ' v/o '.$row['scout_name'], $gL10n->get('SYS_ACTIVE_MEMBERS'));                          
                        }
                        else
                        {
                    ...
                    elseif($gSettingsManager->getBool('mail_show_former'))
                    {
                        if(strlen($row['scout_name'])!=0)
                        {
                           $passiveList[]  = array($usrId, $row['last_name'].' '.$row['first_name']. ' v/o '.$row['scout_name'], $gL10n->get('SYS_FORMER_MEMBERS'));                          
                        }
                        else
                        {
